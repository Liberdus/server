...
;(async () => {
  ...
  async function syncParameters (timestamp, remote) {
    ...
    if (remote) {
      ...
      account = await dapp.getLocalOrRemoteAccount(networkAccount)
    }
    ...
    if (account && account.data) {
      ...
      DATA.NEXT = {}
      DATA.WINDOWS = { proposalWindow, votingWindow, graceWindow, applyWindow }
      DATA.NEXT_WINDOWS = {}
      DATA.ISSUE = 1
      if (dapp.p2p.isFirstSeed) DATA.IN_SYNC = true
      else DATA.IN_SYNC = false
      return DATA
    }
  }

  async function syncDevParameters (timestamp, remote) {
    let account
    if (remote) {
      account = await dapp.getRemoteAccount(networkAccount)
    } else {
    ...
    if (account && account.data) {
      ...
      DEV_DATA = {}
      DEV_DATA.DEV_WINDOWS = { devProposalWindow, devVotingWindow, devGraceWindow, devApplyWindow }
      DEV_DATA.NEXT_DEV_WINDOWS = {}
      DEV_DATA.DEVELOPER_FUND = []
      DEV_DATA.NEXT_DEVELOPER_FUND = []
      DEV_DATA.DEV_ISSUE = 1
      if (dapp.p2p.isFirstSeed) DEV_DATA.IN_SYNC = true
      else DEV_DATA.IN_SYNC = false
      return DEV_DATA
    }
  }
  ...
  dapp.registerExternalGet('network/windows/all', async (req, res) => {
    try {
      res.json({
        windows: DATA.WINDOWS,
        devWindows: DEV_DATA.DEV_WINDOWS
      })
    } catch (error) {
      res.json({ error })
    }
  ...
  dapp.registerExternalGet('issues/dev', async (req, res) => {
    try {
      const devIssues = []
      for (let i = 1; i <= DEV_DATA.DEV_ISSUE; i++) {
  ...
  dapp.registerExternalGet('issues/dev/latest', async (req, res) => {
    try {
      const devIssue = await dapp.getLocalOrRemoteAccount(
        crypto.hash(`dev-issue-${DEV_DATA.DEV_ISSUE}`)
      )
  ...
  dapp.registerExternalGet('issues/dev/count', async (req, res) => {
    try {
      res.json({ count: DEV_DATA.DEV_ISSUE })
    } catch (error) {
      dapp.log(error)
      res.json({ error })
    }
  ...
  dapp.registerExternalGet('proposals/latest', async (req, res) => {
    try {
      ...
      for (let i = 1; i <= proposalCount; i++) {
        let proposal = await dapp.getLocalOrRemoteAccount(
          crypto.hash(`issue-${DATA.ISSUE}-proposal-${i}`)
        )
        if (proposal && proposal.data) {
          proposals.push(proposal.data)
        }
  ...
  dapp.registerExternalGet('proposals/dev/latest', async (req, res) => {
    try {
      ...
      for (let i = 1; i <= devProposalCount; i++) {
        let devProposal = await dapp.getLocalOrRemoteAccount(
          crypto.hash(`dev-issue-${DEV_DATA.DEV_ISSUE}-dev-proposal-${i}`)
        )
        if (devProposal && devProposal.data) {
          devProposals.push(devProposal.data)
        }
  ...
  dapp.setup({
    async sync () {
      if (dapp.p2p.isFirstSeed) {
        if (!DATA) {
          ...
          DEV_DATA = await syncDevParameters(timestamp)
          ...
          dapp.set({
            ...
            from: address,
            to: networkAccount,
            issue: crypto.hash(`issue-${DATA.ISSUE}`),
            proposal: crypto.hash(`issue-${DATA.ISSUE}-proposal-1`),
            timestamp: Date.now()
          })
          ...
          dapp.set({
            ...
            from: address,
            to: networkAccount,
            devIssue: crypto.hash(`dev-issue-${DEV_DATA.DEV_ISSUE}`),
            timestamp: Date.now()
          })
        ...
        try {
          ...
          while (!DATA.IN_SYNC || !DEV_DATA.IN_SYNC) {
            await _sleep(1000)
            timestamp = Date.now()
            DATA = await syncParameters(timestamp, true)
            DEV_DATA = await syncDevParameters(timestamp, true)
    ...
    validateTransaction (tx, wrappedStates) {
      ...
      switch (tx.type) {
        ...
        case 'transfer': {
          ...
          if (from.data.balance < tx.amount + DATA.CURRENT.transactionFee) {
            response.reason =
        ...
        case 'distribute': {
          ...
          if (
            from.data.balance <
            recipients.length * tx.amount + DATA.CURRENT.transactionFee
          ) {
            response.reason =
              "from account doesn't have sufficient balance to cover the transaction"
            return response
        ...
        case 'message': {
          ...
          if (to.data.friends[tx.from]) {
            ...
            if (from.data.balance < to.data.toll + DATA.CURRENT.transactionFee) {
              response.reason = 'from account does not have sufficient funds.'
              return response
            }
          }
        ...
        case 'toll': {
          ...
          if (from.data.balance < DATA.CURRENT.transactionFee) {
            response.reason =
        ...
        case 'stake': {
          ...
          if (from.data.balance < DATA.CURRENT.stakeRequired) {
            response.reason = `From account has insufficient balance, the cost required to operate a node is ${DATA.CURRENT.stakeRequired}`
            return response
          }
          if (tx.stake < DATA.CURRENT.stakeRequired) {
            response.reason = `Stake amount sent: ${tx.stake} is less than the cost required to operate a node: ${DATA.CURRENT.stakeRequired}`
            return response
          }
          response.result = 'pass'
          response.reason = 'This transaction is valid!'
        ...
        case 'node_reward': {
          ...
          if (from) {
            ...
            if (tx.timestamp - from.nodeRewardTime < DATA.CURRENT.nodeRewardInterval) {
              response.reason = 'Too early for this node to get paid'
              return response
            }
          }
        ...
        case 'proposal': {
          ...
          if (
            tx.proposal !==
            crypto.hash(`issue-${DATA.ISSUE}-proposal-${issue.proposalCount + 1}`)
          ) {
            response.reason = 'Must give the next issue proposalCount hash'
            return response
          }
          if (from.data.balance < DATA.CURRENT.proposalFee + DATA.CURRENT.transactionFee) {
            response.reason =
        ...
        case 'dev_proposal': {
          ...
          if (
            tx.devProposal !==
            crypto.hash(
              `dev-issue-${DEV_DATA.DEV_ISSUE}-dev-proposal-${devIssue.devProposalCount +
                1}`
            )
          ) {
          ...
          if (from.data.balance < DATA.CURRENT.devProposalFee + DATA.CURRENT.transactionFee) {
            response.reason =
    ...
    validateTxnFields (tx) {
      ...
      switch (tx.type) {
        ...
        case 'proposal': {
          ...
          if (
            tx.timestamp < DATA.WINDOWS.proposalWindow[0] ||
            tx.timestamp > DATA.WINDOWS.proposalWindow[1]
          ) {
            result = 'fail'
        ...
        case 'dev_proposal': {
          ...
          if (
            tx.timestamp < DEV_DATA.DEV_WINDOWS.devProposalWindow[0] ||
            tx.timestamp > DEV_DATA.DEV_WINDOWS.devProposalWindow[1]
          ) {
            result = 'fail'
        ...
        case 'vote': {
          ...
          if (
            tx.timestamp < DATA.WINDOWS.votingWindow[0] ||
            tx.timestamp > DATA.WINDOWS.votingWindow[1]
          ) {
            result = 'fail'
        ...
        case 'dev_vote': {
          ...
          if (
            tx.timestamp < DEV_DATA.DEV_WINDOWS.devVotingWindow[0] ||
            tx.timestamp > DEV_DATA.DEV_WINDOWS.devVotingWindow[1]
          ) {
            result = 'fail'
    ...
    apply (tx, wrappedStates) {
      ...
      switch (tx.type) {
        ...
        case 'transfer': {
          from.data.balance -= tx.amount + DATA.CURRENT.transactionFee
          from.data.balance -= maintenanceAmount(tx.timestamp, from)
          to.data.balance += tx.amount
          from.data.transactions.push({ ...tx, txId })
          to.data.transactions.push({ ...tx, txId })
        ...
        case 'distribute': {
          ...
          from.data.balance -= DATA.CURRENT.transactionFee
          // from.data.transactions.push({ ...tx, txId })
          recipients.forEach(recipient => {
        ...
        case 'node_reward': {
          to.balance += DATA.CURRENT.nodeRewardAmount
        ...
        case 'proposal': {
          ...
          from.data.balance -= DATA.CURRENT.proposalFee
          from.data.balance -= DATA.CURRENT.transactionFee
          from.data.balance -= maintenanceAmount(tx.timestamp, from)
        ...
        case 'dev_proposal': {
          ...
          from.data.balance -= DATA.CURRENT.devProposalFee
          from.data.balance -= DATA.CURRENT.transactionFee
          from.data.balance -= maintenanceAmount(tx.timestamp, from)
  ...
  async function networkMaintenance(
    ...
    if (
      ...
      if (!apply) {
        ...
        DATA.WINDOWS = DATA.NEXT_WINDOWS
        DATA.NEXT_WINDOWS = {}
    ...
    if (
      ...
      if (!devIssue) {
        if (nodeId === luckyNode && DEV_DATA.DEV_ISSUE >= 2) {
    ...
    if (
      ...
      if (!devApply) {
        ...
        DEV_DATA.NEXT_DEV_WINDOWS = {}
        DEV_DATA.DEVELOPER_FUND = [...DEV_DATA.DEVELOPER_FUND, ...DEV_DATA.NEXT_DEVELOPER_FUND]
        DEV_DATA.NEXT_DEVELOPER_FUND = []
        DEV_DATA.DEV_ISSUE++
        devApply = true
        devIssue = false
    ...
    for (const payment of DEV_DATA.DEVELOPER_FUND) {
      ...
      if (cycleStartTimestamp >= payment.timestamp) {
        ...
        DEV_DATA.DEVELOPER_FUND = DEV_DATA.DEVELOPER_FUND.filter(p => p.id !== payment.id)
      } else {
        break
      }
    }
  ...
  function maintenanceAmount (timestamp, account) {
    let amount
    if (timestamp - account.lastMaintenance < DATA.CURRENT.maintenanceInterval) {
      amount = 0
    } else {
      amount =
        account.data.balance *
        (DATA.CURRENT.maintenanceFee *
          Math.floor(
            (timestamp - account.lastMaintenance) / DATA.CURRENT.maintenanceInterval
          ))
