...
// COMMAND TO SET THE GLOBAL TX FEE
vorpal.command('setTxFee', 'sets the global tx fee').action(async function(_, callback) {
  const answers = await this.prompt([
    {
      type: 'number',
      name: 'fee',
      message: 'What should the global tx fee be:',
      default: 50,
      filter: value => parseInt(value),
    },
  ])
