...
scanner.WriteLines(path.join(scanner.instanceDir, 'playbackMerged.txt'), compositeLines)

// todo aggregate fatal results in a nice way
let fatalLines = scanner.MergeOtherLogs(files, 'fatal')
scanner.WriteLinesRaw(path.join(scanner.instanceDir, 'fatalMerged.txt'), fatalLines)

let errorLines = scanner.MergeOtherLogs(files, 'error')
scanner.WriteLinesRaw(path.join(scanner.instanceDir, 'errorMerged.txt'), errorLines)

let shardDumpLines = scanner.MergeOtherLogsLastLine(files, 'shardDump', 2)
if (shardDumpLines.length > 0) {
  scanner.WriteLinesRaw(path.join(scanner.instanceDir, 'shardDumpMerged.txt'), shardDumpLines)
  scanner.processShardDump(path.join(scanner.instanceDir, 'shardReport.txt'), shardDumpLines)
}
