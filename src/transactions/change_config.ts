...
export const apply = (
  ...
  const network = wrappedStates[networkAccount].data
  ...
  // The apply path normalizes the network account before it lands, so the hash promised
  // here must be taken against the same shape or the archiver sees a mismatch.
  ...
  ourAppDefinedData.globalMsg = { address: networkAccount, addressHash, value, when, source: from.id, afterStateHash }
...
export const keys = (tx: Tx.ChangeConfig, result: ShardusTypes.TransactionKeys): ShardusTypes.TransactionKeys => {
...
export const memoryPattern = (tx: Tx.ChangeConfig, result: ShardusTypes.TransactionKeys): ShardusTypes.ShardusMemoryPatternsInput => {
  return {
    rw: [tx.from],
    wo: [],
    on: [],
